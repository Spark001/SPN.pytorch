...
def read_object_labels(root, year, set):
    path_labels = os.path.join(root, 'VOCdevkit'+year, 'VOC'+year, 'ImageSets', 'Main')
    labeled_data = dict()
...
class Voc2007Classification(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None):
        self.root = root 
        self.path_devkit = os.path.join(root, 'VOCdevkit2007')
        self.path_images = os.path.join(root, 'VOCdevkit2007', 'VOC2007', 'JPEGImages')
        self.set = set
        ...
        if not os.path.exists(file_csv):
            ...
            # generate csv file
            labeled_data = read_object_labels(self.root, '2007', self.set)
            # write csv file
...
class Voc2012Classification(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None):
        self.root = root 
        self.path_devkit = os.path.join(root, 'VOCdevkit2012')
        ...
        if not os.path.exists(file_csv):
            if not os.path.exists(path_csv):  # create dir if necessary
                os.makedirs(path_csv)
            # generate csv file
            labeled_data = read_object_labels(self.root, '2012', self.set)
            # write csv file
            write_object_labels_csv(file_csv, labeled_data)
